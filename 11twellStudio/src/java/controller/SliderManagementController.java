...
public class SliderManagementController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<MovieDTO> movieList = dao.movieList(status, index, 4);
            
            int total = dao.movieList(status, index, -1).size();
            int totalPage = total/4;
            if(total % 4 != 0) totalPage++;
            ...
            request.getRequestDispatcher("slider-management.jsp").forward(request, response);
        } catch (Exception e) {
        }
    }
