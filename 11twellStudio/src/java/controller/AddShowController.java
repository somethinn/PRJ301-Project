...
public class AddShowController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet AddShowController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet AddShowController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<MovieDTO> listMovie = movieDAO.movieList();
        request.setAttribute("listRoom", listRoom);
        request.setAttribute("listSlot", listSlot);
        request.setAttribute("listMovie", listMovie);

        request.getRequestDispatcher("add-show-management.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (showDAO.checkShowExist(showDate, slotID, roomID) == true) {
                response.sendRedirect("show-management");
            } else {
                ShowDTO show = ShowDTO.builder().movie(new MovieDAO().getMovieByID(movieID))
                        .showDate(showDate)
                        .slot(new SlotDAO().getSlotByID(slotID))
